...
class MyTestCase(unittest.TestCase):
    ...
    def test_move_two_pieces_one_space_success_default_settings(self):
        game = initialize_basic_game()
        game.move_piece('ralph', (0, 0), (1, 0), 1)  # 0,0 has nothing and 1, 0 has [R, R]
        game.move_piece('george', (2, 0), (1, 0), 1)  # 2,0 has nothing and 1,0 has [R, R, G]
        game.move_piece('ralph', (5, 0), (4, 0), 1)  # ralph has no more stacks in range of 1,0
        message_success = game.move_piece('george', (1, 0), (2, 0), 1)  # 1,0 has [R, R] and 2,0 has [G]

        self.assertEqual(message_success, MESSAGES['move_success'])
        self.assertListEqual(game.show_pieces((1, 0)), ['R', 'R'])
        self.assertListEqual(game.show_pieces((2, 0)), ['G'])

    def test_attempt_move_three_pieces_four_spaces_default_settings(self):
        game = initialize_basic_game()
        game.move_piece('ralph', (0, 0), (1, 0), 1)  # 0,0 has nothing and 1, 0 has [R, R]
        game.move_piece('george', (2, 0), (1, 0), 1)  # 2,0 has nothing and 1,0 has [R, R, G]
        game.move_piece('ralph', (5, 0), (4, 0), 1)  # 5,0 has nothing and 4,0 has [R, R]
        error_message = game.move_piece('george', (1, 0), (5, 0), 4)  # should return error message and not move

        self.assertEqual(error_message, MESSAGES['invalid_location'])
        self.assertListEqual(game.show_pieces((5, 0)), [])
        self.assertListEqual(game.show_pieces((4, 0)), ['R', 'R'])
        self.assertListEqual(game.show_pieces((1, 0)), ['R', 'R', 'G'])
